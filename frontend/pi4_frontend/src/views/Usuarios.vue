...
export default {
  ...
  methods: {
    ListaUsuarios() {
      ...
      const resposta = await alertUtils.alertConfirmacaoTop(`Deseja ${statusAtual} o usuario ${usuario[0].nome}?`)
      ...
      if(resposta) {
        ...
        await UsuariosService.atualizarEstado(usuario[0].id, novoEstado)
              .then(res => {
                  this.ListaUsuarios();
              }) 
